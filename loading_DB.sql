/* 이름: demo_madang.sql */
/* 설명 */
 
/* root 계정으로 접속, madang 데이터베이스 생성, madang 계정 생성 */
/* MySQL Workbench에서 초기화면에서 +를 눌러 root connection을 만들어 접속한다. */
DROP DATABASE IF EXISTS  loading_DB;
DROP USER IF EXISTS  loading_DB@localhost;
create user loading_DB@localhost identified WITH mysql_native_password  by 'loding';
create database loading_DB;
grant all privileges on loading_DB.* to loading_DB@localhost with grant option;
commit;

/* madang DB 자료 생성 */
/* 이후 실습은 MySQL Workbench에서 초기화면에서 +를 눌러 madang connection을 만들어 접속하여 사용한다. */
 
USE loading_DB;

CREATE TABLE WORKER(
    WORKER_TASK VARCHAR(8) NOT NULL,
    WORKER_DATE DATE NOT NULL,
    WORKER_PERSONNEL INT NOT NULL
);

CREATE TABLE LOGIN(
    LI_PHONENUM VARCHAR(10) NOT NULL,
    LI_NAME VARCHAR(20) NOT NULL,
    LI_UNLOADING VARCHAR(30) NOT NULL,
    IP VARCHAR(20) NOT NULL
);

CREATE TABLE SCHEDULE(
    SCHEDULE_IMPORT VARCHAR(30) NOT NULL,
    SCHEDULE_EXPORT VARCHAR(30) NOT NULL,
    SCHEDULE_VESSEL_NAME VARCHAR(30) NOT NULL,
    SCHEDULE_TON VARCHAR(30) NOT NULL
);

CREATE TABLE VESSEL(
    VESSEL_NAME VARCHAR(30) NOT NULL,
    VESSEL_MRN VARCHAR(4) NOT NULL,
    VESSEL_TON INT NOT NULL,
    VESSEL_TYPE VARCHAR(5) NOT NULL,
    VESSEL_SHIPPER VARCHAR(20) NOT NULL
);

CREATE TABLE DECK(
    DECK_TON INT NOT NULL,
    DECK_FLOOR VARCHAR(2) NOT NULL,
    DECK_VESSEL_NAME VARCHAR(30) NOT NULL,
    HOLD_NO VARCHAR(2) NOT NULL
);

CREATE TABLE CARGO(
    CARGO_VIN VARCHAR(20) NOT NULL,
    CAR_VIN VARCHAR(6) NOT NULL,
    VESSEL_NAME VARCHAR(30) NOT NULL,
    CARGO_NAME VARCHAR(30) NOT NULL,
    CARGO_WEIGHT VARCHAR(30) NOT NULL,
    CARGO_INSPECT_TIME TIMESTAMP(0) NOT NULL,
    IP VARCHAR(20) NOT NULL,
    LI_PHONENUM VARCHAR(10) NOT NULL
);

CREATE TABLE CAR(
    CAR_VIN VARCHAR(6) NOT NULL,
    CAR_NAME VARCHAR(15) NOT NULL,
    CAR_WEIGHT INT NOT NULL,
    CAR_MODEL_DRIVE VARCHAR(50) NOT NULL,
    CAR_BODY_TYPE VARCHAR(260) NOT NULL,
    CAR_BODY_TRIM VARCHAR(20) NOT NULL,
    CAR_GVWR VARCHAR(60) NOT NULL,
    CAR_ENGINE VARCHAR(280) NOT NULL
);

ALTER TABLE WORKER ADD CONSTRAINT PK_WORKER PRIMARY KEY (WORKER_TASK);
ALTER TABLE LOGIN ADD CONSTRAINT PK_LOGIN PRIMARY KEY (IP);
ALTER TABLE SCHEDULE ADD CONSTRAINT PK_SCHEDULE PRIMARY KEY (SCHEDULE_IMPORT, SCHEDULE_VESSEL_NAME);
ALTER TABLE VESSEL ADD CONSTRAINT PK_VESSEL PRIMARY KEY (VESSEL_NAME);
ALTER TABLE DECK ADD CONSTRAINT PK_DECK PRIMARY KEY (DECK_TON,DECK_FLOOR,DECK_VESSEL_NAME);
ALTER TABLE CARGO ADD CONSTRAINT PK_CARGO PRIMARY KEY (CARGO_VIN);
ALTER TABLE CAR ADD CONSTRAINT PK_CAR PRIMARY KEY (CAR_VIN);

ALTER TABLE DECK ADD CONSTRAINT FK_VESSEL1 FOREIGN KEY (DECK_VESSEL_NAME) REFERENCES VESSEL(VESSEL_NAME);
-- ALTER TABLE SCHEDULE ADD CONSTRAINT FK_VESSEL2 FOREIGN KEY (SCHEDULE_VESSEL_NAME) REFERENCES VESSEL(VESSEL_NAME);
ALTER TABLE CARGO ADD CONSTRAINT FK_VESSEL3 FOREIGN KEY (VESSEL_NAME) REFERENCES VESSEL(VESSEL_NAME);
ALTER TABLE CARGO ADD CONSTRAINT FK_CAR1 FOREIGN KEY (CAR_VIN) REFERENCES CAR(CAR_VIN);
ALTER TABLE CARGO ADD CONSTRAINT FK_LOGIN1 FOREIGN KEY (IP) REFERENCES LOGIN(IP);